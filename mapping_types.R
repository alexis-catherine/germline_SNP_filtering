...
pheno <- read.csv("type_mutation_rate_Apr_not_final.csv")
...
for(i in 1:ncol(srd$pheno)){
  plot(x = scan_out,
       map = srd$gmap, 
       lodcolumn=i,
       ylim=c(0, max(c(thr$A[i], thr$X[i]))+1),
       main = colnames(srd$pheno)[i])#, round(thr$A[[i]],3)))
  abline(h = thr$A[i], col = "red", lwd = 2)
  abline(h = summary(out_perm,0.1)$A[i], col = "maroon", lwd = 2)
  abline(h = summary(out_perm,0.15)$A[i], col = "darkred", lwd = 2)
  abline(h = 6, col = "purple", lwd = 2)
  print(i)
}

dev.off()
  peak_Mbp <- find_peaks(scan1_output = scan_out, 
                         map = srd$pmap, peakdrop=3, prob=0.95,
                         ) %>% 
    filter(lod > 8,
           chr != "X")
  peak_Mbp$ci_hi <- ifelse(peak_Mbp$ci_lo==peak_Mbp$ci_hi, peak_Mbp$ci_hi+1, peak_Mbp$ci_hi)

  find_peaks(scan1_output = scan_out, 
             map = srd$pmap, peakdrop=3, prob=0.95,
  ) %>% 
    filter(lod > 8,
           chr != "X") %>% 
...
coef_cX <- scan1coef(apr[,peak_Mbp$chr], srd$pheno[,"mut"], kinship_loco[[peak_Mbp$chr]])
# plot_coefCC(coef_cX,
